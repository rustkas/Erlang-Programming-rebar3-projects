...
%% @doc Starts the database server using usrDb as a default 
%% Dets filename. It is there for testing purposes.
...
%% @doc Deletes the ETS tables and closes the Dets file, terminating the database server. It
...
-spec add_usr(PhoneNum, CustId, Plan) -> ok | {error, timeout} when 
		PhoneNum :: pos_integer(),
		CustId :: pos_integer(),
		Plan :: atom().
...
-spec delete_usr(CustId) -> ok | {error, timeout} when 
		CustId :: pos_integer().

delete_usr(CustId) ->
	call({delete_usr, CustId}).

-spec set_service(CustId, Service, Flag) -> ok | {error, instance | timeout} when
		CustId :: pos_integer(),
		Service :: atom(),
		Flag :: atom().
...
-spec set_status(CustId, Status) -> ok | {error, instance | timeout} when 
		CustId :: pos_integer(),
		Status :: atom().
...
%% @doc Will look up a user based on its CustId and return a record of type usr, as defined
...
-spec lookup_id(CustId) -> {ok, #usr{}} | {error, instance} when 
		CustId :: pos_integer().
